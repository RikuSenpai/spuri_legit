#include "logs.hpp"
#include "..//NEW STUFF/damageindicator.hpp"
...
void c_event_logs::defusing(i_game_event* event) noexcept
{
	if (!c_config::get().bomb_planted)
		return;
	const char * hasdefusekit[2] = { "without defuse kit.","with defuse kit." };
		...
		auto userid_id = interfaces::engine->get_player_for_user_id(userid);

		player_info_t userid_info;
		player_info_t info;


		auto m_player = static_cast<player_t *>(interfaces::entity_list->get_client_entity(userid_id));
		...
		std::stringstream ss;
		ss << player_name << " bomb is being defused " << hasdefusekit[event->get_bool("haskit")];
		render::get().draw_text(3, 520, render::get().name_font, "defusing", false, color(255, 255, 255));
		add(ss.str(), color(255, 255, 255, 255));
...
void c_event_logs::event_player_hurt(i_game_event* event) noexcept {
	...
	if (attacker == local_player) {
		...
		{
			damage_indicator_t damage_indicator;
			damage_indicator.iDamage = event->get_int("dmg_health");
			damage_indicator.Player = entity;
			damage_indicator.flEraseTime = local_player->get_tick_base() * interfaces::globals->interval_per_tick + 3.f;
			damage_indicator.bInitialized = false;

			damage_indicators.data.push_back(damage_indicator);
		}
