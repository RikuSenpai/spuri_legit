...
public:
	void run() noexcept;
	void fov_crosshair() noexcept;

	void anti_aim_indicator() noexcept;

	void grenade_pred() noexcept;
	void entity_esp(player_t * entity) noexcept;
	void glow(player_t * entity) noexcept;
	void skeleton(player_t * entity) noexcept;
	void backtrack_skeleton(player_t * entity) noexcept;
	void wolrd_color() noexcept;
	void chams() noexcept;
	void epilypsy() noexcept;
	void misc_chams(const model_render_info_t & info) noexcept;
	void glow() noexcept;
	void draw_arrows() noexcept;
	void rotate_arrow(std::array<vec2_t, 3>& points, float rotation) noexcept;
	void arrows(player_t * player) noexcept;
...
private:
	...
	__forceinline int dot_product(const vec3_t& a, const vec3_t& b)
	{
