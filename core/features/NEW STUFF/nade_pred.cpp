	...
	auto draw_3d_dotted_circle = [](vec3_t position, float points, float radius) {
		...
		for (float a = 0; a < 3.141592654f * 2.0f; a += step) {
			...
			if (interfaces::debug_overlay->world_to_screen(start, start2d))
				render::get().draw_line(start2d.x, start2d.y, start2d.x + 1, start2d.y + 1, color(255, 255, 255));
		}
	};
	...
	if (_predicted) {
		for (auto &p : _points) {
			...
			if (c_math::get().world_to_screen(p.m_start, start) && c_math::get().world_to_screen(p.m_end, end)) {
				...
				render::get().draw_line(start.x, start.y, end.x, end.y, color(10, 255, 10));

				if (p.m_detonate || p.m_plane) {
					;
					render::get().draw_rect(start.x - 2, start.y - 2, 5, 5, p.m_detonate ? color(255, 0, 0) : color(255, 255, 255));
				}
				if (p.m_detonate)
