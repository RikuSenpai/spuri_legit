...
public:
	...
	void __stdcall setup_resent(IDirect3DDevice9* device) {
		...
		colors[ImGuiCol_Header] = ImVec4(0 / 255.f, 100 / 255.f, 200 / 255.f, 1.0f); //multicombo, combo selected item color.
	...
	void apply_fonts() {
		...
		//font_menu = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Verdana.ttf", 14);
		font_menu2 = ImGui::GetIO().Fonts->AddFontFromFileTTF("C:\\Windows\\Fonts\\Verdana.ttf", 16);
		font_tabs = ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(MyFont_compressed_data222, MyFont_compressed_size222, 16.0f);
		font_menu = ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(MyFont_compressed_data222, MyFont_compressed_size222, 14.0f);
	}
