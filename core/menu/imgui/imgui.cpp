...
ImGuiStyle::ImGuiStyle() {
	...
	FramePadding = ImVec2(5, 2);      // Padding within a framed rectangle (used by most widgets)
	FrameRounding = 3.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
	ItemSpacing = ImVec2(7, 1);      // Horizontal and vertical spacing between widgets/lines
	ItemInnerSpacing = ImVec2(1, 1);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
	TouchExtraPadding = ImVec2(0, 0);      // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
	IndentSpacing = 6.0f;            // Horizontal spacing when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
	ColumnsMinSpacing = 12.0f;             // Minimum horizontal spacing between two columns
...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y - 1 + style.FramePadding.y * 0.5, label_size.y - 1 + style.FramePadding.y * 0.5));
	ItemSize(check_bb, style.FramePadding.y);

	ImRect total_bb = check_bb;
	if (label_size.x > 0)
		SameLine(0, style.ItemInnerSpacing.x);
	const ImRect text_bb(window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 3) - ImVec2(0, 2), window->DC.CursorPos + ImVec2(0, style.FramePadding.y - 3) + label_size);
	if (label_size.x > 0)
	{
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg) {
	...
	ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImVec4(0.10f, 0.15f, 0.30f, 1.0f)), true, style.FrameRounding);
